...
import type { GameState } from '$lib/store/store';
import { type Difficulty, boardSizes, type Coords } from '$lib/utils/constants';
import { type GameCell, isMine, isNumberCell, isFlagged } from './cell';
...
export const openCell = (state: GameState, { x, y }: Coords): boolean => {
	...
	while (cellsToVisit.length > 0) {
		...
		if (adjacentMines === 0) {
			const nextCellsToVisit = getAdjacentCoords(state.board, { x, y }).filter(({ x, y }) => {
				const cell = state.board[y][x];
...
export const openAdjacentCells = (state: GameState, { x, y }: Coords): boolean => {
	const cell = state.board[y][x];
	...
	if (isNumberCell(cell) && cell !== 0) {
		...
		if (adjacentFlaggedCellsCount === cell) {
			const cellsToOpen = adjacentCellsCoords
				.map<[GameCell, Coords]>((coords) => [getCell(state.board)(coords), coords])
...
export const initGame = (difficulty: Difficulty) => (state: GameState) => {
	state.status = 'starting';
	state.difficulty = difficulty;
	state.board = createBoard(difficulty);
	state.totalCellsCount = boardSizes[difficulty].height * boardSizes[difficulty].width;
